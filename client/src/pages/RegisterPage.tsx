...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ background: 'var(--bg-dark)' }}>
      <div className="absolute inset-0 opacity-[0.015]" style={{ background: 'repeating-linear-gradient(0deg, transparent, transparent 3px, var(--neon-magenta) 3px, var(--neon-magenta) 4px)' }} />
      <div className="absolute inset-0" style={{
        background: 'radial-gradient(circle at 50% 40%, rgba(255,0,255,0.05) 0%, transparent 60%)',
      }} />
      <div
      ...
      >
        <div className="text-center mb-8">
          <h1 className="text-xl font-pixel mb-3" style={{
            ...
            textShadow: '0 0 10px var(--neon-magenta), 0 0 30px var(--neon-magenta), 0 0 60px var(--neon-magenta)',
          }}>
            SCHOLARHUB
          </h1>
          <div className="flex items-center justify-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse shadow-sm shadow-green-400" />
            <span className="text-xs tracking-wider" style={{ color: 'var(--text-dim)' }}>REGISTRATION TERMINAL</span>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="card-retro p-6 space-y-5" style={{ borderColor: '#2a2a5a' }}>
          <div className="flex items-center gap-2 pb-2 border-b border-white/5">
            ...
            <h2 className="text-xs font-pixel" style={{ color: 'var(--neon-magenta)' }}>CREATE ACCOUNT</h2>
          </div>
          {error && (
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-pixel tracking-wider" style={{ color: 'var(--text-dim)' }}>NAME</label>
          ...
          <p className="text-xs text-center" style={{ color: 'var(--text-dim)' }}>
            ALREADY IN?{' '}
            <Link to="/login" className="hover:underline transition-all" style={{ color: 'var(--neon-cyan)' }}>
              SIGN IN
            </Link>
          </p>
        </form>
